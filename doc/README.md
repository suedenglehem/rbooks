# library-rag — project documentation
...
| Path | What it is |
|---|---|
| `doc/README.md` | this document — the current state of the project |
...
| `doc/project/RUNBOOK.md` | operator runbook for full-library ingestion (launch, stop, recovery, triage) |
| `doc/project/README.md` | the original M0-era README (frozen development artifact) |

Files under `doc/project/` are historical development records — read them for
...
| Archive (SHA-256 content store) | `/mnt/models_sas_ssd/library-rag/archive` |
| Artifacts (vectors, OCR images) | `/mnt/models_sas_ssd/library-rag/artifacts` |
| State (SQLite `library.db`, job logs) | `/mnt/models_sata_ssd/library-rag/state` |
| Qdrant (embedded) | `/mnt/models_sata_ssd/library-rag/qdrant` |
| Models | `/mnt/models_sas_ssd/library-rag/models` |
| Scratch (hard-capped) | `/mnt/models_sata_ssd/library-rag/scratch` |
| Backups (M7) | `/mnt/models_sas_ssd/library-rag/backups` |

Backups live on the same hardware as the data: they recover accidental loss,
not disk failure.
...
./serve.sh status    # serve + worker + fleet (via /system/status when up)
```

`serve.sh` owns `serve.pid` and manages **only** the serve process; the model
fleet is reported, not managed. It refuses to start while an ingest worker is
running.

**Full-library launch** — currently **HELD on explicit operator approval**.
The approved launch sequence, monitoring, stop/resume, crash recovery and
failure triage are in `doc/project/RUNBOOK.md` (M7 gate deliverable).
...
tests/               pytest suite
serve.sh             operator control script for the serve process
...
doc/project/         frozen development-stage artifacts (PRD, progress, report, runbook)
```
